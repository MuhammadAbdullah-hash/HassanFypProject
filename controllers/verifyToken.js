...
verifyTokenController.post('/token', async (req, res) => {
    ...
    try{
        ...
        if(resultVerify.role == data.role || resultVerify.role == "admin" ){ res.send({"status" : true}) }
        else{ res.send({"status" : false , "message" : "Message You Are not authorized to view this page"}) }
    }
